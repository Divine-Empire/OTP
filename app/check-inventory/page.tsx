...
export default function CheckInventoryPage() {
      ...
      if (data && data.table && data.table.rows) {
        ...
        data.table.rows.slice(6).forEach((row, index) => {
          if (row.c) {
            ...
            // Column BG (index 58) - Planned2 - must not be null
            const hasColumnBG = row.c[58] && row.c[58].v !== null && row.c[58].v !== ""
            // Column BH (index 59) - Actual2 - must be null for pending
            const isColumnBHEmpty = !row.c[59] || row.c[59].v === null || row.c[59].v === ""

            // For pending orders: show rows where Planned2 (BG) is not null but Actual2 (BH) is empty
            if (hasColumnBG && isColumnBHEmpty) {
              const order = {
                ...
                inventoryStatus: row.c[61] ? row.c[61].v : null, // Column BJ (index 61) - Availability Status
                inventoryRemarks: row.c[62] ? row.c[62].v : "", // Column BK (index 62) - Remarks
                processedDate: row.c[59] ? row.c[59].v : "", // Column BH (index 59) - Actual2
                fullRowData: row.c,
              }
      ...
      if (data && data.table && data.table.rows) {
        ...
        data.table.rows.slice(6).forEach((row, index) => {
          if (row.c) {
            ...
            // Column BG (index 58) and BH (index 59) - both must not be null
            const hasColumnBG = row.c[58] && row.c[58].v !== null && row.c[58].v !== ""
            const hasColumnBH = row.c[59] && row.c[59].v !== null && row.c[59].v !== ""

            // For history orders: show rows where both Planned2 (BG) and Actual2 (BH) have data
            if (hasColumnBG && hasColumnBH) {
              const processedOrder = {
                ...
                availabilityStatus: row.c[61] ? row.c[61].v : "", // Column BJ (index 61) - Availability Status
                inventoryRemarks: row.c[62] ? row.c[62].v : "", // Column BK (index 62) - Remarks
                // Keep old field names for backward compatibility
                id: row.c[1] ? row.c[1].v : "",
                contactPerson: row.c[4] ? row.c[4].v : "",
                processedDate: formatGoogleSheetsDate(row.c[59] ? row.c[59].v : ""), // Column BH (index 59) - Actual2
                fullRowData: row.c,
              }
  ...
  const updateOrderStatus = async (order, inventoryData) => {
    try {
      ...
      const rowData = new Array(70).fill(""); // BG(58) to BR(69)
      ...
      // Set processed date (column BH - index 59) - Actual2
      rowData[59] = formattedDate;

      // Set availability status (column BJ - index 61)
      rowData[61] = inventoryData.availabilityStatus;

      // Set remarks (column BK - index 62)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (availabilityStatus === "Not Available" || availabilityStatus === "Partial") {
        rowData[63] = partialDetails.customerDecision || ""; // BL - customer wants material as
        rowData[64] = partialDetails.createdBy || "";        // BM - Created by
        rowData[65] = partialDetails.warehouseLocation || ""; // BN - warehouse location
        rowData[66] = partialDetails.createIndent ? "File Uploaded" : ""; // BO - create indent
        rowData[67] = partialDetails.lineItemNumber || "";   // BP - line item number
        rowData[68] = partialDetails.totalQty || "";         // BQ - total qty
        rowData[69] = partialDetails.leadTime || "";         // BR - Material received lead time
      }
