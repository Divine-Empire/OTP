...
export default function WarehouseMaterialPage() {
      ...
      if (data && data.table && data.table.rows) {
        ...
        data.table.rows.slice(6).forEach((row, index) => {
          if (row.c) {
            ...
            const ceColumn = row.c[82] ? row.c[82].v : null // Column CE (index 82)
            const cfColumn = row.c[83] ? row.c[83].v : null // Column CF (index 83)
          ...
          if (row.c) {
            ...
            if (ceColumn && cfColumn) {
              const order = {
                ...
                fullRowData: row.c,
                materialRcvdData: {
                  materialReceived: row.c[85] ? row.c[85].v : "", // Column CH (index 85)
                  installationRequired: row.c[86] ? row.c[86].v : "", // Column CI (index 86)
                  transporterFollowup: row.c[87] ? row.c[87].v : "", // Column CJ (index 87)
  ...
  const updateOrderStatus = async (order: any) => {
    try {
      ...
      rowData[83] = formattedDate

      // Add material data to columns CH onwards (indexes 85-87)
      rowData[85] = materialReceived // Column CH
      rowData[87] = installationRequired // Column CI
      rowData[86] = transporterFollowup // Column CJ
